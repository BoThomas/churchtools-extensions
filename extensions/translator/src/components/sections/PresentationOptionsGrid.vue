<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    >
      <template #info>
        <p class="text-sm">
          Font size of the translated text. You can specify the size in any CSS
          unit (px, em, rem...).
        </p>
      </template>
    </InputTextField>
    ...
    >
      <template #info>
        <p class="text-sm">
          Color of the translated text. You can specify colors with html names,
          rgb, and hex.
        </p>
      </template>
    </InputTextField>
    ...
    <div class="flex flex-col gap-2 md:pl-6">
      ...
      <div class="flex items-center gap-2 md:h-[42px]">
        <Checkbox
          ...
          @update:model-value="updatePresentation('showInputLanguage', $event)"
          :binary="true"
          :disabled="disabled"
        />
        <label
      ...
      <Popover ref="showInputLangPopover">
        <div class="max-w-sm">
          <p class="text-sm">
            When enabled, the original spoken input will be displayed alongside
            the translations in the presentation view. In split-screen mode, the
            input will appear as an additional pane. In multi-window mode, a
            separate window for the input language will be opened.
          </p>
        </div>
      </Popover>
    </div>
